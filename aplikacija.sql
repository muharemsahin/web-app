...
CREATE TABLE IF NOT EXISTS `article` (
  ...
  CONSTRAINT `fk_article_category_id` FOREIGN KEY (`article_id`) REFERENCES `category` (`category_id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8_unicode_ci;
...
-- Dumping structure for table aplikacija.oder
DROP TABLE IF EXISTS `oder`;
CREATE TABLE IF NOT EXISTS `oder` (
  `oder_id` int unsigned NOT NULL AUTO_INCREMENT,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `cart_id` int unsigned NOT NULL DEFAULT '0',
...
-- Dumping data for table aplikacija.oder: ~0 rows (approximately)
DELETE FROM `oder`;
...
categorycategoryaplikacijaaplikacijaaplikacija